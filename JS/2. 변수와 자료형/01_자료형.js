...
function typeTest2(){
    ...
    // let testFunc = function(num1, num2){
        ...
        // , undefined로 초기화되어 오류가 발생 ==> 해결을 위해 초기값 지정
    // 초기값 지정
    ...
    area2.innerHTML += `undefined type : ${typeof(user)}`;
}

// 2. 데이터 형변환
// 1) 문자열과 숫자의 '+' 연산
//      문자형과 + 연산시 문자열로 변경
//      그외의 연산시 숫자타입인 경우 숫자로 변환하여 연산처리
function plusTest(){
    let test1 = 7 + 7; // 14
    let test2 = '7' + 7; // '77'
    let test3 = 7 + '7'; // '77'
...
function forTest(){
    ...
    for(key in student){
        ...
        // 객체명.속성명 -> 속성값을 알고 있을때 사용
        // 속성명이 key인 요소를 찾는데 없으니까 undefined
