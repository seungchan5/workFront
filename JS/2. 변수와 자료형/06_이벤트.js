window.onload = function(){
    ...
    btn1.onclick = function(e){
        ...
        // 이전 이벤트와 현재 이벤트가 발생한 시간의 차리를 밀리 세컨드로 반환
        console.log(e.code);        // 키보드 키코드 값
        console.log(e.key);         // 키보드 키
    }

    // 고전 방식으로 이벤트를 적용 할 경우
    // 속성에 값을 주는 것이기 때문에
    // 중복적용이 불가능 => 나중에 정의한 함수로 덮어쓰기  
    btn1.onclick = (e) => {
        // 화살표 함수에서는 this가 없음
        // 상위 요소의 this를 찾아서 반환 
    ...
    submit.addEventListener('click', function(e){
        ...
        // 정규식
        // 특정 규칙을 가진 문자열을 검색하거나 치환할때 사용하는 형식 언어
        // /패턴/으로 선언
        // /정규식패턴/.test('문자열');
